      ...
      if (resp.status === 200) {
        ...
        if (serial) {
          try {
            ...
            // The profile generated by ADEEnrollmentProfileService is plain XML
            // (ADE_SIGNING_ENABLED is false).  Return the raw unsigned XML.
            // iOS trusts the HTTPS connection for initial DEP profile delivery,
            // so CMS signing should not be required.
            let body = mobileconfig;
            logger.info(`[MDM Proxy] Returning unsigned profile for ${serial} (${body.length}B)`);
