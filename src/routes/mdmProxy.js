      ...
      if (resp.status === 200) {
        ...
        if (serial) {
          try {
            ...
            // The generated profile is unsigned (ADE_SIGNING_ENABLED is false).
            // CMS-sign it with the mdm-identity cert so iOS accepts it.
            let body = mobileconfig;
            if (mdmSigningKey && mdmSigningCert) {
              try {
                const p7 = forge.pkcs7.createSignedData();
                p7.content = forge.util.createBuffer(mobileconfig);
                p7.addCertificate(mdmSigningCert);
                const caCertPath = environment.ade.rootCaCertPath;
                if (caCertPath) {
                  try {
                    const caPem = fs.readFileSync(path.resolve(caCertPath), 'utf8');
                ...
                p7.sign({ detached: false });
                body = Buffer.from(forge.asn1.toDer(p7.toAsn1()).getBytes(), 'binary');
                logger.info(`[MDM Proxy] CMS-signed profile for ${serial} (${body.length}B)`);
              } catch (signErr) {
                logger.warn(
                  `[MDM Proxy] CMS signing failed, returning unsigned: ${signErr.message}`,
                );
              }
            }
